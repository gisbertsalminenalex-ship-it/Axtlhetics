...
- Vercel for web hosting/deployment (D-014) — the page is prerendered as static HTML and served as an installable PWA with offline support; no static export (`output: 'export'`), because the route below needs a server
- A single Next.js route handler as the only server-side piece: `app/api/axis-ai/route.ts` (Node.js runtime, `maxDuration = 10`), the AXIS language layer, deployed by Vercel as a function
- Groq (OpenAI-compatible REST via `fetch`, no SDK, model `openai/gpt-oss-120b` in `AXIS_AI_MODEL`, free plan) as the AXIS language provider behind that route
...
Do not send personal or training data to external services unless the feature explicitly requires it and the behavior has been approved. The only approved exception is the AXIS language layer (§11): the browser sends a trimmed briefing, without the user's name, to the same-origin route `/api/axis-ai`, which forwards it to the language provider (Groq). Nothing is stored server-side.
...
- PWA on Vercel, installable and usable offline.
...
- Groq as the AXIS language layer through the `/api/axis-ai` route handler, with deterministic fallback.
