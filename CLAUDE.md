...
Axtlhetics Master Document / Documento Maestro.

Decisions explicitly approved later by Alex.

This CLAUDE.md.

Existing code and technical documentation.
...
PLAN — inspect the relevant project and understand the goal.

PROPOSAL — explain architecture, files, dependencies and risks.

APPROVAL — obtain confirmation for important decisions.

IMPLEMENTATION — make the smallest clean change that solves the task.

TEST — run the relevant tests, checks and production build when appropriate.

REVIEW — critically inspect the implementation for bugs, regressions, unnecessary complexity and architectural problems.

REPORT — explain what changed, what was tested and what remains.
